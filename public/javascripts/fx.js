...
// Fx.Move, Sep 2009 by Sinan Taifour (staiii@gmail.com)
...
Fx.Move.prototype = Object.extend(new Fx.Base(), {
  ...
  go: function(value, dirs){
    this.dirs = dirs;
    this.init = {'top': this.element.style['top'], 'left': this.element.style['left']};
    if (this.init['top']) {
      this.init['top'] = parseInt(this.init['top'].substr(0, this.init['top'].length - this.options.unit.length), 10);
    } else {
      this.init['top'] = 0;
    }
    if (this.init['left']) {
      this.init['left'] = parseInt(this.init['left'].substr(0, this.init['left'].length - this.options.unit.length), 10);
    } else {
      this.init['left'] = 0;
    }
    this.now = 0;
    return this.custom(0, value);
  },

  increase: function(){
    if (this.dirs[0] == 'top' || this.dirs[1] == 'top') {
      this.setStyle(this.element, 'top', this.init['top'] + this.now);
    }
    if (this.dirs[0] == 'left' || this.dirs[1] == 'left') {
      this.setStyle(this.element, 'left', this.init['left'] + this.now);
    }
  }
